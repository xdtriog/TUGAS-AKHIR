...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'simpan_mutasi') {
    ...
    $jumlah_rusak_pieces = isset($_POST['jumlah_rusak']) ? intval($_POST['jumlah_rusak']) : 0;
    ...
    try {
        ...
        if (abs($jumlah_rusak_pieces) > $jumlah_sistem_pieces) {
            throw new Exception('Jumlah mutasi tidak boleh melebihi stock sistem!');
        }
        
        // Hitung stock akhir (dalam PIECES)
        $stock_akhir_pieces = $jumlah_sistem_pieces - $jumlah_rusak_pieces;
        if ($stock_akhir_pieces < 0) {
            $stock_akhir_pieces = 0;
        }
        
        // Untuk MUTASI_BARANG_RUSAK toko:
        // JUMLAH_MUTASI_DUS: tidak perlu diisi (NULL atau 0)
        // TOTAL_BARANG_PIECES: jumlah mutasi dalam pieces (negatif karena rusak)
        $jumlah_mutasi_dus = NULL; // Tidak perlu diisi untuk toko
        $total_barang_pieces = -$jumlah_rusak_pieces; // Negatif karena mengurangi stock
        ...
        // Untuk toko: JUMLAH_MUTASI_DUS tidak perlu diisi (NULL)
        ...
        // Update STOCK dengan stock akhir (dalam PIECES)
        ...
        $stmt_update_stock->bind_param("isss", $stock_akhir_pieces, $user_id, $kd_barang, $kd_lokasi);
        ...
        // JUMLAH_PERUBAHAN: -JUMLAH_RUSAK (dalam PIECES, negatif karena mengurangi stock)
        // JUMLAH_AKHIR: STOCK_AKHIR (dalam PIECES)
        // SATUAN: 'PIECES'
        $jumlah_awal_history = $jumlah_sistem_pieces; // JUMLAH_SISTEM dalam PIECES
        $jumlah_perubahan_history = -$jumlah_rusak_pieces; // Negatif karena mengurangi stock
        $jumlah_akhir_history = $stock_akhir_pieces; // STOCK_AKHIR dalam PIECES
...
<head>
    ...
    <title>Toko - Mutasi Barang Rusak</title>
    <!-- Bootstrap CSS -->
...
<body class="dashboard-body">
    ...
    <div class="main-content">
        ...
        <div class="page-header">
            <h1 class="page-title">Toko <?php echo htmlspecialchars($nama_lokasi); ?> - Mutasi Barang Rusak</h1>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('#mutasi_stock_sekarang').val(numberFormat(stockSistemPieces));
                        $('#mutasi_jumlah_rusak').val('');
                        $('#mutasi_jumlah_rusak').attr('max', stockSistemPieces);
                        $('#mutasi_stock_akhir').val(numberFormat(stockSistemPieces));
                        
                        // Store data untuk perhitungan
                        $('#mutasi_stock_sekarang').data('stock-sistem-pieces', stockSistemPieces);
        ...
        function hitungStockAkhir() {
            var stockSistemPieces = parseInt($('#mutasi_stock_sekarang').data('stock-sistem-pieces')) || 0;
            var jumlahRusak = parseInt($('#mutasi_jumlah_rusak').val()) || 0;
            
            // Validasi: jumlah rusak tidak boleh melebihi stock sistem
            if (jumlahRusak > stockSistemPieces) {
                $('#mutasi_jumlah_rusak').val(stockSistemPieces);
                jumlahRusak = stockSistemPieces;
            }
            
            // Hitung stock akhir
            var stockAkhirPieces = stockSistemPieces - jumlahRusak;
            if (stockAkhirPieces < 0) {
                stockAkhirPieces = 0;
            }
            
            // Update tampilan
            $('#mutasi_stock_akhir').val(numberFormat(stockAkhirPieces));
        }
