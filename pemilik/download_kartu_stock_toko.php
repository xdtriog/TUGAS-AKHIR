...
if ($result_barang->num_rows == 0) {
    header("Location: kartu_stock_toko.php?kd_lokasi=" . urlencode($kd_lokasi));
    exit();
}
...
<head>
    ...
    <title>Download Kartu Stock Toko - <?php echo htmlspecialchars($barang['NAMA_BARANG']); ?></title>
    <style>
        @media print { body { margin:0; padding:20px; } .no-print{display:none!important;} .print-container{page-break-after:avoid;} @page{margin:2cm;} }
        body { font-family: Arial, sans-serif; padding: 20px; max-width: 1200px; margin: 0 auto; background-color: #f5f5f5; }
        .header { text-align: center; margin-bottom: 30px; }
        .header h2 { margin: 0; font-size: 24px; font-weight: bold; }
        .header h3 { margin: 5px 0; font-size: 20px; font-weight: bold; }
        .header p { margin: 5px 0; font-size: 14px; }
        .info-section { margin-bottom: 20px; border: 1px solid #000; padding: 15px; background-color: white; }
        .info-section .row { margin-bottom: 10px; }
        .info-section label { font-weight: bold; display: inline-block; width: 150px; }
        .summary-section { display: flex; justify-content: space-around; margin-bottom: 30px; flex-wrap: wrap; }
        .summary-card { border: 1px solid #000; padding: 15px; text-align: center; min-width: 200px; margin: 5px; background-color: white; }
        .summary-card .value { font-size: 1.5em; font-weight: bold; margin-bottom: 5px; }
        .summary-card .label { font-size: 0.9em; color: #666; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 16px; font-size: 10.5px; table-layout: fixed; }
        table th, table td { border: 1px solid #000; padding: 6px; text-align: left; word-wrap: break-word; }
        table th { background-color: #f0f0f0; font-weight: bold; }
        table td.text-center { text-align: center; }
        table td.text-right { text-align: right; }
        .btn-print { background-color: #667eea; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-size: 16px; margin-bottom: 20px; }
        .btn-print:hover { background-color: #5568d3; }
        .footer-summary { margin-top: 30px; text-align: right; }
        .footer-summary p { margin: 5px 0; }
        .footer-summary .total { font-size: 1.2em; font-weight: bold; }
    </style>
</head>
<body>
    ...
    <div class="print-container">
        ...
        <div class="info-section">
            <div class="row"><label>Kode Barang:</label> <?php echo htmlspecialchars($barang['KD_BARANG']); ?></div>
            <div class="row"><label>Nama Barang:</label> <?php echo htmlspecialchars($barang['NAMA_BARANG']); ?></div>
            <div class="row"><label>Merek:</label> <?php echo htmlspecialchars($barang['NAMA_MEREK']); ?></div>
            <div class="row"><label>Kategori:</label> <?php echo htmlspecialchars($barang['NAMA_KATEGORI']); ?></div>
            <div class="row"><label>Periode:</label> <?php echo formatTanggal($tanggal_dari); ?> s/d <?php echo formatTanggal($tanggal_sampai); ?></div>
            <div class="row"><label>Stock Saat Ini:</label> <strong><?php echo number_format($barang['STOCK_SEKARANG'], 0, ',', '.'); ?> <?php echo htmlspecialchars($barang['SATUAN']); ?></strong></div>
        </div>
        <div class="summary-section">
            <div class="summary-card"><div class="value"><?php echo number_format($stock_awal, 0, ',', '.'); ?></div><div class="label">Stock Awal Periode</div></div>
            <div class="summary-card"><div class="value"><?php echo number_format($total_masuk, 0, ',', '.'); ?></div><div class="label">Total Masuk</div></div>
            <div class="summary-card"><div class="value"><?php echo number_format($total_keluar, 0, ',', '.'); ?></div><div class="label">Total Keluar</div></div>
            <div class="summary-card"><div class="value"><?php echo number_format($stock_akhir_periode, 0, ',', '.'); ?></div><div class="label">Stock Akhir Periode</div></div>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php if (count($stock_history) > 0): $no=1; foreach ($stock_history as $h): ?>
                    <tr>
                ...
                <?php endforeach; else: ?>
                    <tr><td colspan="10" class="text-center">Tidak ada pergerakan stock pada periode yang dipilih</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
        ...
        <div style="margin-top: 30px; text-align: center; color: #666; font-size: 12px;">
            <p>Laporan dicetak pada: <?php echo date('d/m/Y H:i') . ' WIB'; ?></p>
        </div>
    </div>
</body>
</html>
