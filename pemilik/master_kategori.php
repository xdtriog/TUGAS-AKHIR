...
// Function untuk generate kode kategori random
function generateKodeKategori($conn) {
    ...
    do {
        ...
        $attempt++;
        if (!checkUUIDExists($conn, 'MASTER_KATEGORI_BARANG', 'KD_KATEGORI_BARANG', $kode)) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'tambah_kategori') {
        ...
        if (!empty($nama_kategori)) {
            ...
            $kd_kategori = generateKodeKategori($conn);
            if ($kd_kategori === false) {
                $message = 'Gagal generate kode kategori! Silakan coba lagi.';
                $message_type = 'danger';
            } else {
                $status = 'AKTIF'; // Status langsung AKTIF
            
            // Insert data
            $insert_query = "INSERT INTO MASTER_KATEGORI_BARANG (KD_KATEGORI_BARANG, NAMA_KATEGORI, STATUS) VALUES (?, ?, ?)";
            $insert_stmt = $conn->prepare($insert_query);
            $insert_stmt->bind_param("sss", $kd_kategori, $nama_kategori, $status);
            
                if ($insert_stmt->execute()) {
                    $message = 'Kategori berhasil ditambahkan dengan kode: ' . $kd_kategori;
                    $message_type = 'success';
                    // Redirect untuk mencegah resubmission
                    header("Location: master_kategori.php?success=1&kd_kategori=" . urlencode($kd_kategori));
                    exit();
                } else {
        ...
        if (!empty($kd_kategori) && !empty($nama_kategori) && !empty($status)) {
            // Update data
            $update_query = "UPDATE MASTER_KATEGORI_BARANG SET NAMA_KATEGORI = ?, STATUS = ? WHERE KD_KATEGORI_BARANG = ?";
            $update_stmt = $conn->prepare($update_query);
            $update_stmt->bind_param("sss", $nama_kategori, $status, $kd_kategori);
            
            if ($update_stmt->execute()) {
                $message = 'Kategori berhasil diperbarui';
                $message_type = 'success';
                // Redirect untuk mencegah resubmission
                header("Location: master_kategori.php?success=2&kd_kategori=" . urlencode($kd_kategori));
                exit();
            } else {
...
if (isset($_GET['success']) && $_GET['success'] == '1') {
    $message = 'Kategori berhasil ditambahkan dengan kode: ' . htmlspecialchars($_GET['kd_kategori'] ?? '');
    $message_type = 'success';
...
$query_kategori = "SELECT KD_KATEGORI_BARANG, NAMA_KATEGORI, STATUS 
                   FROM MASTER_KATEGORI_BARANG 
                   ORDER BY KD_KATEGORI_BARANG ASC";
$result_kategori = $conn->query($query_kategori);

// Set active page untuk sidebar
$active_page = 'master_kategori';
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Pemilik - Master Kategori</title>
    <!-- Bootstrap CSS -->
...
<body class="dashboard-body">
    ...
    <div class="main-content">
        ...
        <div class="page-header">
            <h1 class="page-title">Pemilik - Kategori Barang</h1>
        </div>
        ...
        <div class="table-section">
            <div class="table-responsive">
                <table id="tableKategori" class="table table-custom table-striped table-hover">
                    <thead>
                        <tr>
                            <th>Kode Kategori</th>
                            <th>Nama Kategori</th>
                    ...
                    <tbody>
                        <?php if ($result_kategori->num_rows > 0): ?>
                            <?php while ($row = $result_kategori->fetch_assoc()): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($row['KD_KATEGORI_BARANG']); ?></td>
                                    ...
                                    <td>
                                        <button class="btn-view" onclick="editKategori('<?php echo htmlspecialchars($row['KD_KATEGORI_BARANG']); ?>', '<?php echo htmlspecialchars($row['NAMA_KATEGORI'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($row['STATUS']); ?>')">Edit</button>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php endif; ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                    <h5 class="modal-title" id="modalTambahKategoriLabel">Tambahkan Kategori</h5>
                ...
                <form id="formTambahKategori" method="POST" action="">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="nama_kategori" name="nama_kategori" placeholder="Masukkan nama kategori" maxlength="100" required autofocus>
                            <small class="text-muted">Maksimal 100 karakter. Kode kategori akan dibuat otomatis.</small>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <form id="formEditKategori" method="POST" action="">
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="kd_kategori" id="edit_kd_kategori">
                        
                        <div class="mb-3">
                            <label for="edit_kd_kategori_display" class="form-label">Kode Kategori</label>
                            <input type="text" class="form-control" id="edit_kd_kategori_display" readonly style="background-color: #e9ecef;">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="edit_nama_kategori" name="nama_kategori" placeholder="Masukkan nama kategori" maxlength="100" required autofocus>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#tableKategori').DataTable({
                ...
                order: [[0, 'asc']], // Sort by Kode Kategori
                columnDefs: [
        ...
        $('#modalTambahKategori').on('hidden.bs.modal', function () {
            ...
            isSubmitting = false;
            var submitBtn = $('#formTambahKategori').find('button[type="submit"]');
            submitBtn.prop('disabled', false);
        ...
        function editKategori(kdKategori, namaKategori, status) {
            // Set nilai form edit
            $('#edit_kd_kategori').val(kdKategori);
            $('#edit_kd_kategori_display').val(kdKategori);
            $('#edit_nama_kategori').val(namaKategori);
            $('#edit_status').val(status);
            
            // Reset flag
            isSubmittingEdit = false;
            var submitBtn = $('#formEditKategori').find('button[type="submit"]');
            submitBtn.prop('disabled', false);
            submitBtn.html('Simpan Perubahan');
            
            // Buka modal
            var modalEdit = new bootstrap.Modal(document.getElementById('modalEditKategori'));
            modalEdit.show();
        }
        ...
        $('#formEditKategori').on('submit', function(e) {
            ...
            var namaKategori = $('#edit_nama_kategori').val().trim();
